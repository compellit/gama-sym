"""
Simple lexical syllabification of Galician words, based on onset maximization. 
...
Some added fixes: Resyllabification for some issues found when processing.
"""
...
def search_stress_mark(sylls: list) -> int:
    """
    ...
    Args:
        sylls (list): list of syllables as str
    Returns:
...
def search_stressed_syll(sylls: list) -> bool:
    """
...
def mark_stress(sylls: list[str], diacritic: str = "´", disambiguate: bool = False) -> tuple[str, str, str, int]:
    """
    ...
    Args:
        ...
        disambiguate (bool): If True, adds a stress mark to final syllables with a falling
    ...
    if len(sylls) == 1:
        ...
        # (get no stress mark in final stressed syllable)
            ...
            if disambiguate and len(sylls) > 1:
                sylls[last] = ut._disambiguate_syll_stress(sylls[last], sylls)
                sylls_diac = copy(sylls) # to update after disambiguate
...
def _resyllabify_double_l(sl: list) -> list:
    ...
    for idx, sy in enumerate(sl):
        try:
            # Fix missyllabification cases where a single "l" is output
            # as a syllable
...
def syllabify_full(word, diacritic="´", disambiguate_stress=False):
    """
    ...
    for m in wordre:
        ...
        wupper, wdiac, worig, stressposi = mark_stress(sylls_post, diacritic=diacritic, disambiguate=disambiguate_stress)
        out += wupper + " "
